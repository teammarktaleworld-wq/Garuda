...
export default function OfferHero({
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        ...
        trackEnquirySubmit(formData.car || vehicle);
  ...
  const handleGetOfferClick = useCallback(() => {
    trackGetOfferClick("offer_hero");
    scrollTo(offerSectionId);
  }, [offerSectionId, scrollTo]);

  const handleBookTestDriveClick = useCallback(() => {
    trackBookTestDriveClick("offer_hero");
    openContactTab("testdrive");
  }, [openContactTab]);
