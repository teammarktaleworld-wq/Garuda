...
import type { OutletData } from "@/app/config/showrooms";
...
interface ShowroomProps {
  ...
  /**
   * Primary showroom for the current URL.
   *
   * Example:
   * /Tata-Motors-Narela
   * → Narela showroom
...
function InfoRow({
  ...
  return (
    <div className="flex items-start gap-3 p-3.5 sm:p-4 rounded-xl bg-gray-50 border border-gray-100 transition-colors hover:bg-gray-100/50">
      <Icon
        size={16}
        className="text-[#004b8d] mt-0.5 flex-shrink-0"
        strokeWidth={2.5}
      />

      <div className="min-w-0">
        <p className="text-[9.5px] sm:text-[10px] font-bold tracking-[0.16em] text-gray-400 uppercase mb-0.5">
          {label}
        </p>

        <div className="text-gray-700 text-[12.5px] sm:text-[13px] font-medium leading-relaxed">
          {children}
        </div>
      </div>
    </div>
...
function OutletCard({
  ...
  const displayPhone = outlet.phone.replace(
    /(\d{5})(\d{5})/,
    "$1 $2"
  );
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-[1fr_1.1fr] gap-5 sm:gap-6">
      ...
      <div className="flex flex-col justify-between gap-4 sm:gap-5">
        <div>
          ...
          {/* Location name is rendered as a heading for SEO */}
          <h3 className="text-gray-900 font-black text-[1.25rem] sm:text-[1.35rem] lg:text-[1.5rem] tracking-tight leading-tight mb-4">
            {outlet.name}
          </h3>
          ...
          <div className="space-y-2.5">
            ...
            >
              <a
                href={`tel:+91${outlet.phone}`}
                className="hover:text-[#004b8d] transition-colors duration-150"
              >
                +91 {displayPhone}
              </a>
            </InfoRow>
            ...
            >
              <a
                ...
                className="hover:text-[#004b8d] transition-colors duration-150 break-all"
              >
                {outlet.email}
              </a>
            </InfoRow>
...
export default function Showroom({
  ...
  /* ---------------------------------------------------------------
     Separate showrooms and workshops
  ---------------------------------------------------------------- */

  const showroomOutlets = outlets.filter(
    (o) => o.type === "showroom"
  );

  const workshopOutlets = outlets.filter(
    (o) => o.type === "workshop"
  );
  ...
  /* ---------------------------------------------------------------
     Find the current URL's primary showroom.
     If primaryOutlet is supplied, show it first.
  ---------------------------------------------------------------- */

  const orderedShowroomOutlets = [...showroomOutlets];

  if (primaryOutlet) {
    const primaryIndex = orderedShowroomOutlets.findIndex(
      (outlet) => outlet.id === primaryOutlet.id
    );

    if (primaryIndex > 0) {
      const [primary] = orderedShowroomOutlets.splice(
        primaryIndex,
        1
      );

      orderedShowroomOutlets.unshift(primary);
    }
  }
  ...
  /* ---------------------------------------------------------------
     Reset current card when tab changes
  ---------------------------------------------------------------- */
  ...
  const currentOutlet =
    activeOutlets[current];

  /* ---------------------------------------------------------------
     Prevent rendering invalid outlet state
  ---------------------------------------------------------------- */
  ...
  return (
    <section
      id="showroom"
      className="relative bg-gray-50 py-16 sm:py-20 lg:py-28 overflow-hidden font-sans"
    >
      {/* Background */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            {primaryOutlet
              ? `Visit ${primaryOutlet.name}, your authorised Tata Motors showroom in ${cityLabel}, New Delhi. Get directions, contact our team and explore Tata cars and services.`
              : hasWorkshops
                ? `${showroomOutlets.length} showroom${
                    showroomOutlets.length > 1
                      ? "s"
                      : ""
                  } and ${workshopOutlets.length} service centre${
                    workshopOutlets.length > 1
                      ? "s"
                      : ""
                  } in ${cityLabel}.`
                : `Your authorised Tata Motors showroom in ${cityLabel}.`}
        ...
        >
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-white border border-gray-200 hover:bg-gray-50 hover:border-gray-300 flex items-center justify-center text-gray-500 hover:text-[#004b8d] transition-all duration-200 shadow-sm"
                >
                  <ChevronLeft
                    size={16}
                    strokeWidth={2.5}
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={`${tab}-${current}-${currentOutlet.id}`}
              initial={
                prefersReduced
                  ? false
                  : { opacity: 0, x: 20 }
